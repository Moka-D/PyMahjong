...
class Shoupai:
    ...
    def valid_mianzi(m: str):
        ...
        if re.search(r'^z.*[089]', m):  # 字牌に0, 8, 9があればエラー
            return None

        h = m.replace('0', '5')     # 赤牌(0)を黒牌(5)に正規化

        if re.search(r'^[mpsz](\d)\1\1[\+\=\-]\1?$', h):    # 刻子か槓子の場合
            # 先頭の'05'を'50'に
    ...
    def __str__(self) -> str:
        ...
        # ツモ牌がある場合は最後に追加する
        if self._zimo is not None and len(self._zimo) <= 2:
            paistr += self._zimo
        # リーチ後は牌姿に * を追加する
        if self._lizhi:
            paistr += '*'
